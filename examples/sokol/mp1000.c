...
static struct {
    ...
    double emu_time_ms;
        ...
        mp1000_snapshot_t snapshots[UI_SNAPSHOT_MAX_SLOTS];
    #endif
} state;
...
static void ui_boot_cb(mp1000_t* sys);
...
// get mp1000_desc_t struct
mp1000_desc_t mp1000_desc() {
    return (mp1000_desc_t) {
        .audio = {
...
void app_init(void) {
    ...
    gfx_init(&(gfx_desc_t){
        ...
        .display_info = mp1000_display_info(&state.mp1000),
    });
    ...
    fs_init();
        ...
        ui_init(&(ui_desc_t){
            ...
            .imgui_ini_key = "floooh.chips.mp1000",
        });
...
void app_frame(void) {
    ...
    state.ticks = mp1000_exec(&state.mp1000, state.frame_time_us);
    state.emu_time_ms = stm_ms(stm_since(emu_start_time));
    draw_status_bar();
    gfx_draw(mp1000_display_info(&state.mp1000));
    handle_file_loading();
    send_keybuf_input();
}
...
void app_cleanup(void) {
    mp1000_discard(&state.mp1000);
    #ifdef CHIPS_USE_UI
        ui_mp1000_discard(&state.ui);
        ui_discard();
    #endif
    saudio_shutdown();
    gfx_shutdown();
...
static void send_keybuf_input(void) {
    ...
    if (0 != (key_code = keybuf_get(state.frame_time_us))) {
        /* FIXME: this is ugly */
        mp1000_key_down(&state.mp1000, key_code);
        mp1000_key_up(&state.mp1000, key_code);
    }
}
...
#if defined(CHIPS_USE_UI)
static void ui_draw_cb(const ui_draw_info_t* draw_info) {
    ui_mp1000_draw(&state.ui, &(ui_mp1000_frame_t){
        .display = draw_info->display,
    });
}

static void ui_save_settings_cb(ui_settings_t* settings) {
    ui_mp1000_save_settings(&state.ui, settings);
}

static void ui_boot_cb(mp1000_t* sys) {
    clock_init();
    mp1000_desc_t desc = mp1000_desc();
    mp1000_init(sys, &desc);
}

static void ui_update_snapshot_screenshot(size_t slot) {
    ui_snapshot_screenshot_t screenshot = {
        .texture = ui_create_screenshot_texture(mp1000_display_info(&state.snapshots[slot].mp1000))
    };
...
static void ui_save_snapshot(size_t slot) {
    if (slot < UI_SNAPSHOT_MAX_SLOTS) {
        state.snapshots[slot].version = mp1000_save_snapshot(&state.mp1000, &state.snapshots[slot].mp1000);
        ui_update_snapshot_screenshot(slot);
        fs_save_snapshot("mp1000", slot, (chips_range_t){ .ptr = &state.snapshots[slot], sizeof(mp1000_snapshot_t) });
    }
}
...
static bool ui_load_snapshot(size_t slot) {
    ...
    if ((slot < UI_SNAPSHOT_MAX_SLOTS) && (state.ui.snapshot.slots[slot].valid)) {
        success = mp1000_load_snapshot(&state.mp1000, state.snapshots[slot].version, &state.snapshots[slot].mp1000);
    }
    return success;
}
...
static void ui_fetch_snapshot_callback(const fs_snapshot_response_t* response) {
    ...
    if (response->data.size != sizeof(mp1000_snapshot_t)) {
        return;
    }
    if (((mp1000_snapshot_t*)response->data.ptr)->version != MP1000_SNAPSHOT_VERSION) {
        return;
    }
...
static void ui_load_snapshots_from_storage(void) {
    for (size_t snapshot_slot = 0; snapshot_slot < UI_SNAPSHOT_MAX_SLOTS; snapshot_slot++) {
        fs_load_snapshot_async("mp1000", snapshot_slot, ui_fetch_snapshot_callback);
    }
}
...
/*    if (data.size <= sizeof(webapi_fileheader_t)) {
    ...
    bool loaded = mp1000_quickload(&state.mp1000, prg);
    if (loaded) {
...
static void web_dbg_read_memory(uint16_t addr, int num_bytes, uint8_t* dst_ptr) {
    for (int i = 0; i < num_bytes; i++) {
        *dst_ptr++ = mem_rd(&state.mp1000.mem_cpu, addr++);
    }
}
#endif
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    const chips_display_info_t info = mp1000_display_info(0);
    return (sapp_desc) {
        .init_cb = app_init,
        .frame_cb = app_frame,
        .event_cb = app_input,
